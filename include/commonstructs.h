...
struct PerInstanceData {
    ...
    getAttributeDescriptions() {
        ...
        attributeDescriptions[0].location = static_cast<size_t>(
            VertexAttributesLocations::INSTANCE_MAT_COLUMN_0);
        attributeDescriptions[0].format = VK_FORMAT_R32G32B32A32_SFLOAT;
        attributeDescriptions[0].offset = 0 * step;

        attributeDescriptions[1].binding = 1;
        attributeDescriptions[1].location = static_cast<size_t>(
            VertexAttributesLocations::INSTANCE_MAT_COLUMN_1);
        attributeDescriptions[1].format = VK_FORMAT_R32G32B32A32_SFLOAT;
        attributeDescriptions[1].offset = 1 * step;

        attributeDescriptions[2].binding = 1;
        attributeDescriptions[2].location = static_cast<size_t>(
            VertexAttributesLocations::INSTANCE_MAT_COLUMN_2);
        attributeDescriptions[2].format = VK_FORMAT_R32G32B32A32_SFLOAT;
        attributeDescriptions[2].offset = 2 * step;

        attributeDescriptions[3].binding = 1;
        attributeDescriptions[3].location = static_cast<size_t>(
            VertexAttributesLocations::INSTANCE_MAT_COLUMN_3);
